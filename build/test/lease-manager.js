...
mocha_1.describe('LeaseManager', () => {
    ...
    mocha_1.describe('instantiation', () => {
        mocha_1.it('should default the bytes value to 0', () => {
            assert.strictEqual(leaseManager.size, 0);
        });
        mocha_1.it('should capture any options passed in', () => {
            const fakeOptions = {};
    ...
    mocha_1.describe('pending', () => {
        mocha_1.it('should return the number of pending messages', () => {
    ...
    mocha_1.describe('size', () => {
        mocha_1.it('should return the number of messages', () => {
            leaseManager.add(new FakeMessage());
            leaseManager.add(new FakeMessage());
    ...
    mocha_1.describe('add', () => {
        mocha_1.it('should update the bytes/size values', () => {
            const message = new FakeMessage();
            leaseManager.add(message);
            assert.strictEqual(leaseManager.size, 1);
            assert.strictEqual(leaseManager.bytes, message.length);
        });
        mocha_1.it('should dispatch the message if allowExcessMessages is true', done => {
        ...
        mocha_1.it('should dispatch the message if the inventory is not full', done => {
            const fakeMessage = new FakeMessage();
        ...
        mocha_1.it('should not dispatch the message if the inventory is full', done => {
            const fakeMessage = new FakeMessage();
            leaseManager.isFull = () => true;
            leaseManager.setOptions({ allowExcessMessages: false });
        ...
        mocha_1.it('should not dispatch the message if the sub closes', done => {
            const fakeMessage = new FakeMessage();
            leaseManager.isFull = () => false;
        ...
        mocha_1.it('should emit the full event if it becomes full', done => {
            leaseManager.setOptions({ allowExcessMessages: false, maxMessages: 1 });
            leaseManager.on('full', done);
            leaseManager.add(new FakeMessage());
        });
        mocha_1.describe('extending deadlines', () => {
            let clock;
            let random;
            let expectedTimeout;
            let halfway;
            ...
            mocha_1.it('should schedule a lease extension', () => {
                const message = new FakeMessage();
                const stub = sandbox
                    .stub(message, 'modAck')
            ...
            mocha_1.it('should not schedule a lease extension if already in progress', () => {
            ...
            mocha_1.it('should remove any messages that pass the maxExtension value', () => {
            ...
            mocha_1.it('should continuously extend the deadlines', () => {
                const message = new FakeMessage();
                const stub = sandbox
                    .stub(message, 'modAck')
                    .withArgs(subscriber.ackDeadline);
    ...
    mocha_1.describe('clear', () => {
        mocha_1.it('should completely clear out the inventory', () => {
            leaseManager.add(new FakeMessage());
            leaseManager.add(new FakeMessage());
            leaseManager.clear();
        ...
        mocha_1.it('should emit the free event if it was full', done => {
            leaseManager.setOptions({ maxMessages: 1 });
            leaseManager.add(new FakeMessage());
            leaseManager.on('free', done);
            setImmediate(() => leaseManager.clear());
        });
        mocha_1.it('should cancel any lease extensions', () => {
    ...
    mocha_1.describe('isFull', () => {
        mocha_1.it('should return true if the maxMessages threshold is hit', () => {
            const maxMessages = 1;
        ...
        mocha_1.it('should return true if the maxBytes threshold is hit', () => {
            const message = new FakeMessage();
        ...
        mocha_1.it('should return false if no thresholds are hit', () => {
            const message = new FakeMessage();
            const maxMessages = 2;
            const maxBytes = message.length + 1;
    ...
    mocha_1.describe('remove', () => {
        mocha_1.it('should noop for unknown messages', () => {
            const message = new FakeMessage();
            leaseManager.add(message);
        ...
        mocha_1.it('should emit the free event if there is free space', done => {
            const message = new FakeMessage();
            leaseManager.setOptions({ maxMessages: 1 });
            leaseManager.add(message);
        ...
        mocha_1.it('should remove a message from the pending state', done => {
        ...
        mocha_1.it('should dispense a pending messages', done => {
        ...
        mocha_1.it('should cancel any extensions if no messages are left', () => {
            const clock = sandbox.useFakeTimers();
            const message = new FakeMessage();
            const stub = sandbox.stub(subscriber, 'modAck').resolves();
            leaseManager.add(message);
    ...
    mocha_1.describe('setOptions', () => {
        mocha_1.it('should allow excess messages by default', () => { });
        mocha_1.it('should default maxBytes to 20% of free memory', () => {
        ...
        mocha_1.it('should cap maxMessages at 100', () => {
            for (let i = 0; i < 100; i++) {
                assert.strictEqual(leaseManager.isFull(), false);
                leaseManager.add(new FakeMessage());
            }
